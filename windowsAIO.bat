...
SET "FILENAME=%SystemDrive%\ffmpeg\ffmpeg.7z"
ECHO.
ECHO Downloading ffmpeg, please wait...
powershell -Command "Invoke-WebRequest https://www.gyan.dev/ffmpeg/builds/ffmpeg-git-full.7z -OutFile '%FILENAME%'"
ECHO.
...
IF EXIST "%SystemDrive%\ffmpeg\ffmpeg.7z" powershell.exe -nologo -noprofile -command "& { Add-Type -A 'System.IO.Compression.FileSystem'; [IO.Compression.ZipFile]::ExtractToDirectory('%SystemDrive%\ffmpeg\ffmpeg.zip"', '%SystemDrive%\ffmpeg\'); }"
ECHO.
